...
class Database:
    ...
    def get_connection(self):
        ...
        try:
            ...
            conn.row_factory = lambda c, r: dict(zip([col[0] for col in c.description], r))  # Permite acceso por nombre de columna
            yield conn
            conn.commit()
    ...
    def calculate_points(self, user_id: int, tiempo: int, fiesta: int = 0) -> tuple[int, int]:
        ...
        if account == "VIP":
            ...
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("UPDATE USUARIOS SET puntos = ? WHERE userID = ?", (nuevos_puntos, user_id))
                cursor.execute("UPDATE USUARIOS SET wallet = ? WHERE userID = ?", (nuevos_creditos, user_id))
